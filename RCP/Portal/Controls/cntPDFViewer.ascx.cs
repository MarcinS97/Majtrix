...
{
    public partial class cntPDFViewer : System.Web.UI.UserControl
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                //    DataRow dr = db.getDataRow(con, String.Format("select top 1 Id from SqlMenu where Grupa = '{0}' and Kolejnosc = 1 order by Id desc", Grupa));
                ...
                btEdit.Visible = App.User.IsPortalAdmin;//App.User.HasRight(AppUser.rPortalArticles);
                Init();
            }
        }
        //----------------------
        ...
        public string GetGazetka(out string file)
        {
            SqlConnection con = db.Connect(db.PORTAL);
            DataRow dr = db.getDataRow(con, String.Format("select top 1 * from SqlMenu where Grupa = '{0}' and Kolejnosc = 1 order by Id desc", Grupa));
            db.Disconnect(con);
            if (dr != null)
            {
        ...
        {
            SqlConnection con = db.Connect(db.PORTAL);
            bool ok = db.update(con, "SqlMenu", 1, "Command", "Grupa='{0}' and Kolejnosc = 1", Grupa, db.nullStrParam(cmd));
            if (!ok)
                ok = db.insert(con, "SqlMenu", 0, "Grupa,MenuText,Command,Kolejnosc", db.strParam(Grupa), db.strParam("PDF"), db.nullStrParam(cmd), 1);
            db.Disconnect(con);
            return ok;
        }
        //-----------------------------------
