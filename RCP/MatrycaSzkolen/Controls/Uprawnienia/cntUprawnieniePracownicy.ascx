<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="cntUprawnieniePracownicy.ascx.cs" Inherits="HRRcp.MatrycaSzkolen.Controls.Uprawnienia.cntUprawnieniePracownicy" %>
...
<div class="zoomMatryca">
    <asp:Menu ID="tabFilter" runat="server" Orientation="Horizontal" 
        ...
        <Items>
            ...
            <asp:MenuItem Text="Brak" Value="B"></asp:MenuItem>            
        </Items>
        <StaticItemTemplate>
...
left join Uprawnienia U on U.Id = @uprId

--left join Certyfikaty C on C.IdPracownika = P.Id and C.IdUprawnienia = @uprId 
/*
left join VCertyfikatyUprawnienie3 C on C.IdPracownika = P.Id and (U.PoziomId is null and C.IdUprawnienia = @uprId or C.PoziomId = U.PoziomId and C.PoziomPoziom &gt;= U.PoziomPoziom)
...
and (
    @cc = -99 or @cc is null or
    R.Id in (select IdPrzypisania from SplityWspP where IdCC = @cc)
    )
order by 2
    " onselected="SqlDataSource1_Selected">
    <SelectParameters>
        <asp:ControlParameter ControlID="hidUprId" Name="uprId" PropertyName="Value" Type="Int32" />
...
select 
	...
	C.Uwagi
from Pracownicy P
inner join Certyfikaty C on C.IdPracownika = P.Id_Pracownicy and C.IdUprawnienia = @uprId 
and (@awp = '9' or C.Aktualny = 1) 
and (
    @awp = '9' or
    @awp = 'A' and (C.DataWaznosci is null or C.DataWaznosci &gt;= @data) or
    ...
    @awp = 'B' and C.DataWaznosci &lt; @data
    )

where P.Id_Str_OrgM in (select items from dbo.SplitInt(@strList, null)) 
and (
