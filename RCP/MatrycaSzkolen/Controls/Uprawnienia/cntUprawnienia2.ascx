<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="cntUprawnienia2.ascx.cs" Inherits="HRRcp.MatrycaSzkolen.Controls.Uprawnienia.cntUprawnienia2" %>
    ...
    <ItemTemplate>
        <tr class='it <%# Eval("Css") %>'>
            <td class="nazwisko">
                ...
                <%--<asp:LinkButton ID="lbPrzeciw" Text="!!" runat="server" Visible='<%# (String)Eval("Przeciwwskazanie") != String.Empty %>' data-toggle="tooltip" title="aoe" />--%>
                <a id="aPrzeciw" runat="server" Visible='<%# (String)Eval("Przeciwwskazanie") != String.Empty %>' data-toggle="tooltip" title='<%# Eval("Przeciwwskazanie") %>' data-placement="top"><i class="fa fa-warning"></i></a>
            </td>
                    ...
                    <asp:LinkButton ID="lbtCertyfikat" runat="server" CommandName="zoomCert" CommandArgument='<%# GetDataCmd(Container.DataItem) %>' Text='<%# GetData(Container.DataItem) %>' Visible='<%# GetVisible(Container.DataItem) %>' ></asp:LinkButton>
                        <% if(TwoColumn) { %>
                            ...
                            <asp:LinkButton ID="LinkButton16" runat="server" CommandName="zoomOcena" CommandArgument='<%# GetDataCmd(Container.DataItem) %>' Text='<%# GetData2(Container.DataItem) %>' Visible='<%# GetVisible(Container.DataItem) %>'></asp:LinkButton>
                                   <%--<asp:ImageButton ID="LinkButton17" runat="server" CommandName="zoomOcena" CommandArgument='<%# GetDataCmd(Container.DataItem) %>' Text='<%# GetData2(Container.DataItem) %>' Visible='<%# GetVisible(Container.DataItem) %>' ImageUrl='<%# GetGradeImage(Container.DataItem) %>'  />--%>
                        <% } %>
                </ItemTemplate>
            </asp:Repeater> 
            </td>
...
--%>
                            ...
                            <asp:Repeater ID="rpHeader1" runat="server" OnItemCommand="rpUprawnienia_ItemCommand">
                                <ItemTemplate>
                                    </th><th class='data1 xvert<%# GetHeaderClass() %>' colspan='<%# GetColspan %>'  >
...
--%>
                            ...
                            <th rowspan="2" class="suma vert" colspan="1" ><div><span>
...
--%>
                             ...
                             <% if(TwoColumn) { %>
                                    </th><th class='ocena<%# GetHeaderClass() %>'>
...
--:CUT
PV.IdPracownika, PV.Pracownik as Pracownik, ISNULL(msp.Przeciwwskazanie, '''') Przeciwwskazanie, PV.Nazwisko, PV.Imie, PV.Nr_Ewid, PV.Nr_Ewid2, PV.APT, PV.NazwiskoLetter, 
' + @colsH + '
...
, case when ISNULL(msp.Przeciwwskazanie, '''') != '''' then ''przeciwwskazanie'' else '''' end [Css]
, '''' [ToolTip]
, '''' [StrOrg]
...
outer apply (select top 1 * from Certyfikaty 
            ...
            --and ((DataZdobyciaUprawnien &gt;= DATEADD(D,-7,P.DataZatr)) or 1 = 1)
...
left join msPrzeciwwskazania msp on msp.IdPracownika = PV.IdPracownika and dbo.getdate(GETDATE()) between msp.DataOd and ISNULL(msp.DataDo, ''20990909'')
