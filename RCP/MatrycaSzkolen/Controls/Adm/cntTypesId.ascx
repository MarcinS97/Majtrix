<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="cntTypesId.ascx.cs" Inherits="HRRcp.MatrycaSzkolen.Controls.Adm.cntTypesId" %>
...
<div id="ctTypes" runat="server" class="cntTypes">
    ...
    <asp:ListView ID="lvTypes" runat="server" DataSourceID="dsTypes" DataKeyNames="Id" InsertItemPosition="LastItem">
        ...
        <EditItemTemplate>
            <tr class="eit">
                <td>
                    <asp:TextBox ID="TextBox1" runat="server" Text='<%# Bind("Idx") %>' />
                </td>
                <td>
    ...
    <asp:SqlDataSource ID="dsTypes" runat="server" ConnectionString="<%$ ConnectionStrings:HRConnectionString %>"
        SelectCommand="exec('select *, Id as Idx from ' + @TableName)"
        DeleteCommand="exec('delete from ' + @TableName + ' where Id = ' + @Id)"
        InsertCommand="exec('insert into ' + @TableName + ' (Id, Nazwa) values (' + @Id + ', ''' + @Nazwa + ''')')"
        UpdateCommand="exec('update ' + @TableName + ' set Id = ' + @Idx + ', Nazwa = ''' + @Nazwa + ''' where Id = ' + @Id)">
        <SelectParameters>
