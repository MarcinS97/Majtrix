<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="cntPlanPracy2.ascx.cs" Inherits="HRRcp.RCP.Controls.cntPlanPracy2" %>
...
<asp:ListView ID="lvPlanPracy" runat="server" DataKeyNames="Id" DataSourceID="SqlDataSource1" 
    ...
    <ItemTemplate>
        ...
        <tr class='<%# GetLineClass(Eval("Ja")) %>' >
            ...
            <td class="colselect day">
                <asp:Label ID="lblKodPrac" runat="server" Text='<%# Eval("KodPrac") %>' ToolTip='<%# Eval("KodPracOpis") %>' />
            </td>
            <td id="tdR1" class="num tdR1" runat="server" visible="false">
            ...
            <td><asp:Label ID="lblRSum" runat="server" Text='<%# Eval("RSum") %>' /></td>
            <td><asp:Label ID="Label1" runat="server" Text='<%# Eval("PSum") %>' /></td>
            <td><asp:Label ID="Label2" runat="server" Text='<%# Eval("NSum") %>' /></td>
            <td><asp:Label ID="Label3" runat="server" Text='<%# Eval("NDSum") %>' /></td>
            <td><asp:Label ID="Label4" runat="server" Text='<%# Eval("NDLSum") %>' /></td>
    ...
    <div class="grid-wrapper" runat="server" visible="false">
        <asp:GridView ID="gvBottomSums" runat="server" DataSourceID="dsBottomSums" AutoGenerateColumns="true" PageSize="20" AllowPaging="false" AllowSorting="false" CssClass="GridView1 tbPlanPracy">
            <EmptyDataTemplate></EmptyDataTemplate>
        </asp:GridView>
    
    </div>

    <asp:SqlDataSource ID="dsBottomSums" runat="server" ConnectionString="<%$ ConnectionStrings:HRConnectionString %>" CancelSelectOnNullParameter="false"
        SelectCommand="
...
select
  @colsH = isnull(@colsH + ',', '') + 'CONVERT(varchar, ISNULL([' + CONVERT(varchar(10), d.Data, 20) + '] + (case x.x when 0 then 0 when 1 then -15 when 2 then -50 end), '''')) [' + case dbo.dow(d.Data)
    when 0 then 'Pn'
    when 1 then 'Wt'
    when 2 then 'Śr'
    when 3 then 'Cz'
    ...
    when 5 then 'So'
    when 6 then 'Ni'
    end + ';' + CONVERT(varchar, DAY(d.Data)) + '-' + case when MONTH(d.Data) &lt; 10 then '0' else '' end + CONVERT(varchar, MONTH(d.Data)) + ':NS;day' + case when k.Rodzaj is null then '' else ' holiday' end + ']'
, @colsD = isnull(@colsD + ',', '') + '[' + CONVERT(varchar(10), d.Data, 20) + ']'
from dbo.getdates2(@od, @do) d
left join Kalendarz k on k.Data = d.Data

set @stmt = '
declare @od datetime = ''' + CONVERT(varchar(10), @od, 20) + '''
declare @do datetime = ''' + CONVERT(varchar(10), @do, 20) + '''
declare @com int = ' + CONVERT(varchar, @com) + '
declare @kierId int = ' + convert(varchar, @kierId) + '

select
 case x.x when 0 then ''il. Os./zm '' + z.Symbol when 1 then ''il. Os. wtry'' when 2 then ''nadw. na mont'' end [:class]
, case x.x when 0 then ''il. Os./zm '' + z.Symbol when 1 then ''il. Os. wtry'' when 2 then ''nadw. na mont'' end [ :;pracname]
, '''' [   :;day]
, '''' [    :;day]
, ' + @colsH + '
, '''' [  :;day]
...
(
    ...
    (
        ...
        , COUNT(*) c
        from PlanPracy pp
        inner join Przypisania r on r.IdPracownika = pp.IdPracownika and r.IdCommodity = @com /*r.IdKierownika = @kierId*/ and r.Status = 1 and /*@od &lt; isnull(r.Do, ''20990909'') and r.Od &lt; @do*/ pp.Data between r.Od and ISNULL(r.Do, ''20990909'')
        where pp.Data between @od and @do
        group by pp.IdZmianyPlan, pp.Data
    ) pp
    PIVOT
    (
        MAX(pp.c) for pp.Data in (' + @colsD + ')
    ) pv
) t on t.IdZmianyPlan = z.Id
cross join (select 0 x union select 1 union select 2) x
order by z.Kolejnosc * 10 + x.x
'
...
">
    <SelectParameters>
        ...
        <asp:ControlParameter ControlID="hidKId" Name="kierId" PropertyName="Value" Type="String" />

        <asp:ControlParameter ControlID="hidCommodity" Name="com" PropertyName="Value" Type="Int32" />
    </SelectParameters>
</asp:SqlDataSource>
...
(
    select 
      pp.IdPracownika
    , SUM(case when pp.Data &gt;= @od and pp.IdZmianyPlan = 188 then 1 else 0 end) R
    , SUM(case when pp.Data &gt;= @od and pp.IdZmianyPlan = 189 then 1 else 0 end) P
    , SUM(case when pp.Data &gt;= @od and pp.IdZmianyPlan = 190 then 1 else 0 end) N
    ...
    where pp.Data between dateadd(MONTH, -1, @od) and isnull(dateadd(D, 1, @do), '20990909')
...
select
    ...
    , 'Puste' NazwiskoImie
    , null KadryId 
    , cast(0 as bit) Kierownik
    , null StrefaId      -- na koniec okresu
    , null Algorytm 
    , null RcpId 
    , null Status
    , null DataZatr
    ...
    , null KodPracOpis
    */
ORDER BY Sort asc, Ja desc, Kierownik desc, NazwiskoImie
    ">
    <SelectParameters>
        <asp:ControlParameter ControlID="hidFrom"   Name="Od" PropertyName="Value" Type="DateTime" />
        <asp:ControlParameter ControlID="hidTo"     Name="Do" PropertyName="Value" Type="DateTime" />
