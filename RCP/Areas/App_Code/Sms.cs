...
namespace HRRcp.Areas.App_Code
{
    public class Sms
    {
        public static void Send(string msg, string to)
        {
            string log = null;
            string pass = null;
            //DataTable settings2 = DB.GetDataTable("select * from Settings where id=1");
            //foreach (DataRow dr in settings2.Rows)
            //{
            //    log = dr["Login"].ToString();
            //    pass = dr["Password"].ToString();
            //}
            try
            {
                SMSApi.Api.Client client = new SMSApi.Api.Client(log);
