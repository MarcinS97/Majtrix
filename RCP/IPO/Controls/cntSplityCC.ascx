<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="cntSplityCC.ascx.cs" Inherits="HRRcp.IPO.Controls.cntSplityCC" %>
<%@ Register Assembly="AjaxControlToolkit" Namespace="AjaxControlToolkit" TagPrefix="asp" %>

<%--
<asp:HiddenField ID="hidPrevValues" runat="server" />
--%>
<asp:HiddenField ID="hidNoGroups" runat="server" Visible="false"/>
    ...
    <ItemTemplate>
        <tr class="it">
            ...
            <td id="tdWsp" runat="server" class='<%# "num" + IsLastColClass %>'>
                <asp:Label ID="lbWsp" runat="server" Text='<%# Eval("Udzial") %>' Visible='<%# IsReadOnly %>' />
                <asp:TextBox ID="WspTextBox" runat="server" CssClass="sumwsp textbox" MaxLength="5" Text='<%# Eval("Udzial") %>' Visible='<%# !IsReadOnly %>'/>
...
<asp:SqlDataSource ID="SqlDataSource2" runat="server" 
    ...
    SelectCommand="select 0 as Sort, null as Id, 'wybierz ...' as Nazwa union
                   SELECT 1 as Sort, Id, cc + ' - ' + Nazwa as Nazwa 
                   FROM CC 
                   where ISNULL(AktywneDo, GETDATE()) &gt;= GETDATE() and (@noGroup = 0 or Grupa = 0) 
                   ORDER BY Sort, Nazwa">
    <SelectParameters>
        <asp:Parameter DefaultValue="0" Name="noGroup" Type="Int32" />
    </SelectParameters>                   
</asp:SqlDataSource>
