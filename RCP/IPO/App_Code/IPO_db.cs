...
{
    public static class IPO_db
    {
        public static string conStr = ConfigurationManager.ConnectionStrings["IPO"].ConnectionString;
        /*
#if PORTAL
        ...
        */
        ...
        {
            return IPO_db.getScalar(String.Format(sql, par));
        }
        ...
        public static bool backupTable(string tbName, string bckName)
        {
            ...
            return IPO_db.execSQL(String.Format(
        ...
        {
            if (String.IsNullOrEmpty(bckName)) bckName = "copy" + tbName;
            string fields = IPO_db.GetColumns(tr, tbName);
            return IPO_db.execSQL(tr, String.Format(
        ...
        */
        ...
        {
            return CopyTable(IPO_db.con, sql, conDest, toTable, deleteAll);
        }
        //----- 
                                    ...
                                    SqlConnection conDest, string toTable, bool deleteAll)
        {
            ...
            int d = 1;
            DataSet ds = Base.getDataSet(conFrom, sql);
            if (deleteAll) IPO_db.execSQL(conDest, "delete from " + toTable);
            foreach (DataRow dr in IPO_db.getRows(ds))
            {
        ...
        public static string DataSetCompare(DataSet ds1, DataSet ds2)
        {
            ...
            if (!match)
            {
                ...
                for (int i = 0; i < c1; i++)
                {
                    DataRow dr1 = IPO_db.getRow(ds1, i);
                    DataRow dr2 = IPO_db.getRow(ds2, i);
                    bool m = true;
                    for (int c = 0; c < cols; c++)
                        if (IPO_db.getValue(dr1, c) != IPO_db.getValue(dr2, c))
                        {
                            m = false;
                            break;
                        }
