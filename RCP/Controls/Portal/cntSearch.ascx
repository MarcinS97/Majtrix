<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="cntSearch.ascx.cs" Inherits="HRRcp.Controls.Portal.cntSearch" %>
            ...
            <tr class='<%# Eval("css") %>' >
                <td>
                    ...
                    <asp:LinkButton ID="LinkButton1" runat="server" CommandArgument='<%# Eval("CmdPar") %>' CommandName='<%# Eval("Cmd") %>' ToolTip='<%# Eval("ToolTip") %>'>
                        <div id="Div1" class="ico" runat="server" >    <%--visible='< % # IsVisible(Eval("Image")) %>'    GetPath(Eval("Image")) --%>
                            <asp:Image ID="Image1" runat="server" ImageUrl='<%# GetImage(Eval("Image"), "../../images/buttons/select.png") %>' />
                        </div>
                        <div class="text">
