<%@ Page Title="" EnableEventValidation="false" Language="C#" MasterPageFile="~/Scorecards/Report.Master" AutoEventWireup="true" CodeBehind="RapRozdzielnik.aspx.cs" Inherits="HRRcp.Scorecards.Reports.RapRozdzielnik" %>
...
, @date [TrueDate:-]
, Type [Type:-]
, Date [Date:C], [Reporting Date], [Exact GL account code], [Cost Center] [Cost Center], Description, Creditor, Project, SUM(ISNULL(Debit, 0)) as [Debit:NN2S;c3|../Raport 11 @pid @TrueDate @Type], SUM(ISNULL(Credit, 0)) as [Credit:NN2S;c2|../Raport 12 @pid @TrueDate @Type], [Currency], [Costunit code]
from #uuu
group by Date, [Reporting Date], [Exact GL account code], [Cost Center], Description, Creditor, Project, Currency, [Costunit code], IdCC, Type
