<%@ Page Title="" EnableEventValidation="false" Language="C#" MasterPageFile="~/Scorecards/Report.Master" AutoEventWireup="true" CodeBehind="RapProduktywnosc3.aspx.cs" Inherits="HRRcp.Scorecards.Reports.RapProduktywnosc3" %>
...
declare @date datetime = @do--GETDATE()
...
, ta.Nazwa as  [Arkusz]
, soa.cc [MPK M]
, aoe.sum_nominal [Czas nominalny:NN2]
, aoe.sum_prodtime [Czas pracy:NN2]
...
, ISNULL(prem.PremiaMiesieczna, 0) [Premia miesięczna:N0.00S]
--, ISNULL(prem.AbsencjaKorekta, 0) [Premia za dyspozycyjność:N0.00S]
, ISNULL(prem.PremiaUznaniowa, 0) [Premia uznaniowa:N0.00S]
, ISNULL(a.Srednia, 0) [Średnia ilość aparatów:N]
...
--left join scWnioski w on w.IdTypuArkuszy = ta.Id and w.IdPracownika = k.Id and w.Data = dbo.bom(@do)
...
(
	select
	prem.PremiaMiesieczna,
	case when ta.Rodzaj = 4 then prem.PremiaUznaniowa else (select SUM(PremiaUznaniowa) + SUM(AbsencjaKorekta) from scPremie p where p.IdWniosku = w.Id and p._do is null and p.IdPracownika != 0 - ta.Id) end PremiaUznaniowa
	--case when ta.Rodzaj = 4 then NULL else (select SUM(AbsencjaKorekta) from scPremie p where p.IdWniosku = w.Id and p._do is null and p.IdPracownika != 0 - ta.Id) end AbsencjaKorekta
	from scWnioski w
	left join scPremie prem on prem.IdWniosku = w.Id
	where prem.IdPracownika = case when ta.Rodzaj = 4 then pr.Id else 0 - ta.Id end and _do is null and w.IdTypuArkuszy = ta.Id and w.Data = dbo.bom(@do) and prem.GodzProd != 0 and w.Status = 3
) prem

left join #aparaty a on a.IdPracownika = p.IdPracownika
outer apply (select * from dbo.GetProdQCAbs2(p.IdCommodity, case when ta.Rodzaj = 4 then pr.Id else 0 - p.IdCommodity end, @observerId, @od, @do)) aoe
