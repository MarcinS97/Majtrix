<%@ Page Title="" EnableEventValidation="false" Language="C#" MasterPageFile="~/Scorecards/Report.Master" AutoEventWireup="true" CodeBehind="RapAbs.aspx.cs" Inherits="HRRcp.Scorecards.Reports.RapAbs" %>
...
, soa.cc --[MPK]
, sum(/*pp2oa.GodzNieob*/aoa.AbsGodz) as GodzNieob --[Godz. Nieob.|RapAbsZoom @pid @SQL2 @SQL3]
, sum(ppoa.Nominal) as Nominal --[Nomnial]
, (case when sum(ppoa.Nominal) = 0 then 0 else round(sum(aoa.AbsGodz) / sum(ppoa.Nominal) * 100, 2) end) as WskaznikAbs --[Wskaźnik abs.|charts`values]
from dbo.GetDates2(@od, @do) dz
left join dbo.fn_GetSubPrzypisania(@observerId, @data) prz on (dz.Data between prz.Od and ISNULL(prz.Do, '20990909') and prz.Status = 1 and prz.IdCommodity is not null /*PRODUKCYJNI*/)
left join Pracownicy p on prz.IdPracownika = p.Id
left join Pracownicy k on prz.IdKierownika = k.Id
...
outer apply (select dbo.cat(CC.cc, ', ', 0) as cc from SplityWspP s left join CC on s.IdCC = CC.Id where IdPrzypisania = prz.Id) soa
outer apply (select top 1 IdCC from SplityWspP where IdPrzypisania = prz.Id) soa2
where soa2.IdCC = @ccId or @ccId = 0
group by prz.IdPracownika, p.Id, p.Nazwisko, p.Imie, p.KadryId, k.Nazwisko, k.Imie, k.KadryId, soa.cc
order by p.Nazwisko
...
, '' Kierownik
, '' cc
, round(sum(/*pp2oa.GodzNieob*/aoa.AbsGodz), 2) as GodzNieob
, round(sum(ppoa.Nominal), 2) as Nominal
