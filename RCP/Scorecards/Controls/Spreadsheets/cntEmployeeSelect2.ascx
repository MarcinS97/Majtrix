<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="cntEmployeeSelect2.ascx.cs" Inherits="HRRcp.Scorecards.Controls.Spreadsheets.cntEmployeeSelect2" %>
            ...
            <table id="tbTeamSelect" class="tbGroupsSelect hiddenTable">
                ...
                <asp:Repeater ID="rpTeams" runat="server" DataSourceID="dsTeams">
                    <ItemTemplate>
                        <tr class="items">
                            <td>
                                <asp:LinkButton ID="lnkTeam" runat="server" CssClass='<%# GetTeamClass("ziomek", Eval("Id").ToString()) %>' Text='<%# Eval("Name") %>' OnClick="ChangeTeam" CommandArgument='<%# Eval("TeamEmployeeId") %>' />
                            </td>
                        </tr>
                    </ItemTemplate>
                </asp:Repeater>
...
">
                ...
                SelectCommand="
select distinct IdCommodity as Id, CONVERT(varchar, IdCommodity) + ';' + CONVERT(varchar, k.Id) as TeamEmployeeId, ta.Nazwa + case when k.Id != @ObserverId then (', TL: ' + k.Nazwisko + ' ' + k.Imie) else '' end as Name
from dbo.fn_GetTreeOkres(@ObserverId, dbo.bom(@Date), dbo.eom(@Date), @Date) p
left join scTypyArkuszy ta on p.IdCommodity = ta.Id
left join scSlowniki s on ta.Rodzaj = s.Id
left join Pracownicy k on k.Id = p.IdKierownika
